...
import {
  isInstructorMonitoringShellPath,
  normalizePath,
...
function selectedStudentId(pathname: string) {
  const match = pathname.match(/\/instructor\/monitoring\/([^/]+)/);
  return match?.[1] ?? null;
}
...
export function InstructorMonitoringSplit({
  ...
  function applyFilters(event: React.FormEvent) {
    ...
    const base = activeId
      ? `/instructor/monitoring/${activeId}`
  ...
  function openStudent(studentId: string) {
    const query = filterQuery();
    const href = query
      ? `/instructor/monitoring/${studentId}?${query}`
      : `/instructor/monitoring/${studentId}`;
    navigate(href);
  }
  ...
  const pendingInShell =
    isPending &&
    pendingPath != null &&
    isInstructorMonitoringShellPath(pendingPath);
  const detailOpen = pendingInShell
    ? pendingPath !== "/instructor/monitoring"
    : Boolean(activeId);
